...
#       reference: row[0].to_s.split("-")[0],
    #### product_references << row[0].to_s.split("-")[0]
#       price: row[2],
#       deposit: row[4]
#     }
  #### end
...
#### Product.all.zip(product_references).map { |x, y| puts x[:reference].include? y }

# Product.all.each do |product|
#   product_references.select { |ref| puts product[:reference].include? ref }
# end
...
promo[:display] = true
promo.remote_media_url = "https://res.cloudinary.com/dqgpcthzg/image/upload/v1533302764/Loxam-Aff-OPlocation-700x500px.jpg"
promo.save
